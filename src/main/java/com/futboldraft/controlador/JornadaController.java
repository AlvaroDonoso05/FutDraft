...
public class JornadaController {
	...
	private void mostrarEventos(String partido) {
		String resultado1, resultado2, golesLoc, golesVis;
        if (partido != null) {
        	partidos.get(0).setImprimir(false);
        	listViewEventos.getItems().clear();
            listViewEventos.getItems().setAll(eventosPartidos.get(partido));
            resultado1 = eventosPartidos.get(partido).get(17);
            resultado2 = eventosPartidos.get(partido).get(18);
            golesLoc = resultado1.substring(resultado1.indexOf(":") + 2);
            golesVis = resultado2.substring(resultado2.indexOf(":") + 2);
